...
export default vue.defineComponent({
  beforeCreate() {
    console.log("estoy en beforeCreate");
  },
  created() {
    this.todojunto;
  ...
  methods: {
    toLogic(): void {
      this.todojunto = `nombre: ${this.person.name}, apellido: ${
        this.person.surname
      }, edad: ${this.person.age}, random: ${Math.random()}`;
    },
    getRespon(): void {
  ...
  data() {
    return {
      todojunto: "" as string,
      statica: `nombre: ${this.person?.name}, apellido: ${this.person?.surname}, edad: ${
        this.person?.age
      }, random: ${Math.random()}` as string,
      mensaje:
        "nombre:" +
        this.person?.name +
        "," +
  ...
  computed: {
    ...
    metodo(): string {
      return this.counter % 2 === 0
        ? `nombre: ${this.person.name}, edad: ${(this.person.age = Math.random() * 100)}`
        : `apellido: ${this.person.surname}, nombre: ${this.person.name}`;
    },
  },
  ...
  watch: {
    ...
    infoMovie: {
      handler(newValue) {
        console.log(
          `llama a los valores de infoMuvie objeto: ${newValue.tittel} y ${newValue.year}`
        );
      },
      deep: true,
    },
    ...
    concatMovie: {
      handler(newValue) {
        console.log(
          `llama a los valores de concatMovie que crea un nuevo Array ${newValue}`
        );
      },
    },
    math: {
      handler(newValue) {
        if (
          newValue.indexOf("suma") > -1 ||
          newValue.indexOf("resta") > -1 ||
          newValue.indexOf("multiplica") > -1 ||
          newValue.indexOf("dividir") > -1
        ) {
          console.log("estoy pensado");
          this.getRespon();
        }
...
<template>
  ...
  <h3 class="center">
    watch en profundidad, con string, objeto y array (modificando -> necesita atributo
    deep y creando -> sin el atributo deep)
  </h3>
  <input type="text" v-model="movie" />
  <input type="text" v-model="infoMovie.tittel" ref="myInputRef2" />
  <input type="number" v-model="infoMovie.year" ref="myInputRef1" /><br />
  <div class="padding">
    <button @click="listaMovie.push(infoMovie.tittel)">
      modifica array listaMovie -{{ listaMovie.length }}
    </button>
    &nbsp
    <button @click="concatMovie = concatMovie.concat('tesis')">
      crea unevo array concatMovie -{{ concatMovie.length }}
    </button>
  </div>
  <hr />
